...
public class RandomDecorator implements Decorator {
  ...
  public void decorate(Resource resource, DecoratorContext context) {
    ...
    if (Scopes.isBlockUnit(resource)) {
      ...
      context.saveMeasure(ModelbusMetrics.RANDOM, value);

    } else {
      // we sum random values on resources different than method
      context.saveMeasure(ModelbusMetrics.RANDOM, MeasureUtils.sum(true, context.getChildrenMeasures(ModelbusMetrics.RANDOM)));
    }
  }
}
